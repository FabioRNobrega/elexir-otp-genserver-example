defmodule ElhexDelivery.PostalCode.DataParser do
  @postal_codes_filepath "data/2021_Gaz_zcta_national.txt"
  ...
  def parse_data do
    ...
    |> Stream.filter(&valid_data_row?(&1))
    |> Stream.map(&parse_data_columns(&1))
    |> Stream.map(&format_row(&1))
    |> Enum.into(%{})
  end

  defp valid_data_row?(row) do
    case row do
  ...
  # format three element list into two element tuple
  # [postal_code, latitude, longitude] # => { postal_code, {latitude, logitude}}
