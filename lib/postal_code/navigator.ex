defmodule ElhexDelivery.PostalCode.Navigator do
  ...
  defp calculate_distance({lat1, long1}, {lat2, long2}) do
    lat_diff = degrees_to_radius(lat2 - lat1)
    long_diff = degrees_to_radius(long2 - long1)

    lat1 = degrees_to_radius(lat1)
    lat2 = degrees_to_radius(lat2)

    cos_lat1 = Math.cos(lat1)
    cost_lat2 = Math.cos(lat2)

    sin_lat_diff_sq = Math.sin(lat_diff / 2) |> Math.pow(2)
    sin_long_diff_sq = Math.sin(long_diff / 2) |> Math.pow(2)

    a = sin_lat_diff_sq * (cos_lat1 * cost_lat2 * sin_long_diff_sq)
    c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a))

    @radius * c |> Float.round(2)
  end

  defp degrees_to_radius(degrees) do
    degrees * (Math.pi / 100)
  end
end
